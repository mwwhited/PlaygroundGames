...
namespace DrawUserPrimitives
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private void InitializePointList()
        {
            ...
            //    }
            //}
            var file1 = "JohnstownNewark.gpx";
            var file2 = "Track_From_work.gpx";
            var file3 = "Track_To_work.gpx";
            var vectors = file1.AsGpxVectors()
                               .Concat(file2.AsGpxVectors())
                               .Concat(file3.AsGpxVectors());
            var minZ = vectors.Min(v => v.Z);
            var maxZ = vectors.Max(v => v.Z);
            Func<float, float> percentZ = z => (z - minZ) / (maxZ - minZ);
            var minX = vectors.Min(v => v.X);
            var maxX = vectors.Max(v => v.X);
            Func<float, float> percentX = x => (x - minX) / (maxX - minX);
            var minY = vectors.Min(v => v.Y);
            var maxY = vectors.Max(v => v.Y);
            Func<float, float> percentY = y => (y - minY) / (maxY - minY);
            Func<float, Color> getColor = x =>
            {
                ...
                return new Color(
                    (byte)(firstColor.R * p),
                    (byte)(secondColor.G * (1 - p)),
                    (byte)((firstColor.B + secondColor.B) / 2)
                    );
            };
            points = vectors.Count();
            pointList = (from vector in vectors
                         select new VertexPositionColor(new Vector3(percentX(vector.X) * 200,
                                                                    percentY(vector.Y) * 200,
                                                                    vector.Z * 0),
                                                        getColor(vector.Z))
                        ).ToArray();
